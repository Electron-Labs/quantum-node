...
pub async fn generate_auth_token_for_protocol(data: GenerateAuthTokenRequest) -> AnyhowResult<GenerateAuthTokenResponse> {
    let is_present = check_if_protocol_already_registered(get_pool().await.read().await.as_ref().as_ref().unwrap(), &data.protocol_name).await;
    ...
    insert_protocol_auth_token(get_pool().await.read().await.as_ref().as_ref().unwrap(), &data.protocol_name, &token).await?;
