...
pub async fn register_circuit_exec<T: Vkey>(data: RegisterCircuitRequest, config_data: &State<ConfigData>, protocol: Protocol) -> AnyhowResult<RegisterCircuitResponse> {
    ...
    insert_user_circuit_data(&get_pool().await.lock().await.as_ref().expect("DB uninitialized"), &circuit_hash_string, &vkey_path, reduction_circuit_id.clone(), data.num_public_inputs, data.proof_type,if reduction_circuit_id.is_some() {CircuitReductionStatus::Completed} else {CircuitReductionStatus::NotPicked}, &protocol.protocol_name).await?;
...
pub async fn get_circuit_registration_status(circuit_hash: String) -> AnyhowResult<CircuitRegistrationStatusResponse> {
    let user_circuit = get_user_circuit_data_by_circuit_hash(&get_pool().await.lock().await.as_ref().expect("DB uninitialized"), circuit_hash.as_str()).await?;
...
//     let user_circuit = get_user_circuit_data_by_circuit_hash(&get_pool().await.lock().await.as_ref().expect("DB uninitialized"), &circuit_hash).await?;
...
async fn create_circuit_reduction_task(reduction_circuit_id: Option<String>, circuit_hash: &str) -> AnyhowResult<()> {
    if reduction_circuit_id.is_none() {
        task_repository::create_circuit_reduction_task(&get_pool().await.lock().await.as_ref().expect("DB uninitialized"), circuit_hash, TaskType::CircuitReduction , TaskStatus::NotPicked).await?;
    }
    Ok(())
}
...
async fn get_existing_compatible_reduction_circuit(num_public_inputs: u8, proving_scheme: ProvingSchemes) -> Option<ReductionCircuit> {
    ...
    if proving_scheme == ProvingSchemes::Groth16 {
        reduction_circuit =  check_if_pis_len_compatible_reduction_circuit_exist(&get_pool().await.lock().await.as_ref().expect("DB uninitialized"), num_public_inputs, proving_scheme).await;
    }
    reduction_circuit
}
...
async fn check_if_circuit_has_already_registered(circuit_hash_string: &str) -> bool {
    let circuit_data = get_user_circuit_data_by_circuit_hash(&get_pool().await.lock().await.as_ref().expect("DB uninitialized"), circuit_hash_string).await;
