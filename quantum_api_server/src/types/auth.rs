    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        if let Some(auth_header) = request.headers().get_one("Authorization") {
            ...
            if request.uri().path() == "/auth/protocol" {
                is_present =  check_if_auth_token_registered_and_is_master(get_pool().await.read().await.as_ref().as_ref().unwrap(), auth_token).await;
            } else {
                is_present = match get_protocol_by_auth_token(&get_pool().await.read().await.as_ref().as_ref().unwrap(), auth_token).await {
                        Ok(p) => match p {
                            Some(_) => Ok(true),
                            None => Ok(false),
                        },
