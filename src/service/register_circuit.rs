...
use crate::{ config::ConfigData,
    ...
    repository::{reduction_circuit_repository::get_reduction_circuit_by_pis_len, 
        task_repository::create_circuit_reduction_task, 
        user_circuit_data_repository::{get_user_circuit_data, insert_user_circuit_data}},
    types::{ gnark_groth16::GnarkGroth16Vkey, register_circuit::{RegisterCircuitRequest, RegisterCircuitResponse}}, 
    utils::file::{create_dir, dump_json_file}};

pub async fn register_circuit_exec(data: RegisterCircuitRequest, config_data: &State<ConfigData>) -> RegisterCircuitResponse {
    ...
    let gnark_vkey = GnarkGroth16Vkey::deserialize(&mut vkey_bytes.as_slice()).unwrap();
    ...
    if is_circuit_already_registered  {
        ...
        return RegisterCircuitResponse { 
            circuit_hash: circuit_hash_string
        };
    }
    ...
    let reduction_circuit_id = match get_reduction_circuit_by_pis_len(data.num_public_inputs).await {
        Some(rc) => Some(rc.id.unwrap()),
        None => None
    };
    println!("reduction circuit id: {:?}", reduction_circuit_id );

    if reduction_circuit_id.is_none() {
        create_circuit_reduction_task(&circuit_hash_string, TaskType::CircuitReduction , CircuitReductionStatus::NotPicked).await;
    }
    insert_user_circuit_data(&circuit_hash_string, &vk_key_full_path, reduction_circuit_id, data.num_public_inputs ).await;
...
fn dump_vkey(gnark_vkey: GnarkGroth16Vkey, vk_path: &str) {
    create_dir(vk_path);
    dump_json_file(vk_path, "vk.json", gnark_vkey);
}

async fn check_if_circuit_has_already_registered(circuit_hash_string: &str) -> bool {
    let circuit_data = get_user_circuit_data(circuit_hash_string).await;
    println!("circuit data: {:?}", circuit_data);
