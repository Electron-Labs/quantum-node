...
pub async fn get_user_circuit_data_by_circuit_hash(pool: &Pool<Any>, circuit_hash: &str) -> AnyhowResult<UserCircuitData>{
...
pub async fn get_user_circuits_by_circuit_status(pool: &Pool<Any>, status: CircuitReductionStatus) -> AnyhowResult<Vec<UserCircuitData>> {
...
fn get_user_circuit_data_from_mysql_row(row: &AnyRow) -> AnyhowResult<UserCircuitData, AnyhowError>{
    let proving_scheme = match ProvingSchemes::from_str(row.try_get_unchecked("proving_scheme").map_err(|err| anyhow!(error_line!(err)))?) {
        Ok(ps) => Ok(ps),
        Err(e) => Err(anyhow!(CustomError::DB(error_line!(e))))
    };

    let circuit_reduction_status_u8: u8 = row.try_get_unchecked::<i64, &str>("circuit_reduction_status").map_err(|err| anyhow!(error_line!(err)))?.try_into()?;
...
pub async fn update_user_circuit_data_reduction_status(pool: &Pool<Any>, user_circuit_hash: &str, status: CircuitReductionStatus) -> AnyhowResult<()> {
    let query  = sqlx::query("UPDATE user_circuit_data set circuit_reduction_status = ? where circuit_hash = ?")
                .bind(status.as_u8() as i64).bind(user_circuit_hash);
...
pub async fn update_user_circuit_data_redn_circuit(pool: &Pool<Any>, user_circuit_hash: &str, reduction_circuit_id: &str) -> AnyhowResult<()> {
