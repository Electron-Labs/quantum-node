...
use quantum_circuits_interface::ffi::interactor::{get_init_tree_data, QuantumV2CircuitInteractor};
use quantum_db::repository::{
    reduction_circuit_repository::get_reduction_circuit_for_user_circuit,
    superproof_repository::{
        get_last_superproof, get_last_verified_superproof, update_superproof_agg_time,
        update_superproof_leaves_path, update_superproof_proof_path, update_superproof_root,
    },
};
use quantum_types::{
    traits::{circuit_interactor::CircuitInteractorFFI, pis::Pis, proof::Proof, vkey::Vkey},
    types::{
        aggregator::{AggregatorCircuitData, IMTLeaves, InnerCircuitData}, config::ConfigData, db::proof::Proof as DBProof, gnark_groth16::{GnarkGroth16Pis, GnarkGroth16Proof, GnarkGroth16Vkey}, hash::KeccakHashOut, imt::IMT_Tree
    },
};
use quantum_utils::{
    ...
    paths::{
        get_aggregation_circuit_proving_key_path, get_aggregation_circuit_vkey_path,
        get_superproof_leaves_path, get_superproof_proof_path,
    },
};
use sqlx::{MySql, Pool};
...
pub async fn handle_imt_aggregation(
    pool: &Pool<MySql>,
    proofs: Vec<DBProof>,
    superproof_id: u64,
    config: &ConfigData,
    ...
    if last_updated_superproof.is_some() {
        ...
        if last_superproof.id.unwrap() == superproof_id {
            ...
            last_root = KeccakHashOut(decode_keccak_hex(
                &last_superproof.superproof_root.unwrap(),
            )?);
...
#[cfg(test)]
mod tests {
    use quantum_circuits_interface::ffi::interactor::get_init_tree_data;
