...
pub async fn register_circuit(
    ...
    let request = registration::handle_registration_task(registration_task.clone(), config).await;
...
pub async fn generate_reduced_proof(
    ...
    let request = proof_generator::handle_proof_generation_task(proof_generation_task.clone(), config).await;
